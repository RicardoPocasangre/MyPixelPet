...
int leerEntero()
{
    ...
    while (true)
    {
        ...
        if (cin.eof())
            return 5;                                     // si el usuario manda la señal de fin del archivo el programa devuelve el valor 5 para guardar y salir
...
bool menuCargarPartida(Mascota &m, const string especies[])
{
    ...
    if (slot == 0)
        return false; // el jugador decidio volver sin cargar
...
// Dibuja una barra de progreso animada caracter por caracter.
// Ejemplo de salida: [||||||    ] 60%
// La animacion se logra imprimiendo cada '|' con una pausa de RETARDO_ANIMACION ms.
void dibujarBarra(int valor)
{
    // Calcula cuantos segmentos se llenan segun el porcentaje
    int llenos = (valor * LARGO_BARRA) / 100;

    cout << "[";
    for (int i = 0; i < LARGO_BARRA; i++)
    {
        cout << (i < llenos ? "|" : " "); // '|' si esta lleno, ' ' si esta vacio
        cout.flush();                     // fuerza que se imprima de inmediato
        pausar(RETARDO_ANIMACION);        // pausa breve para el efecto animado
    }
    cout << "] " << valor << "%";
}
...

{
    if (valor >= UMBRAL_ADVERTENCIA)
        return ""; // sin peligro: no muestra nada

    // Mensaje personalizado segun que estadistica esta en peligro
    if (idx == 0)
        return " <-- Advertencia: Mucha Hambre!"; // aca es por si el hambre esta abaja y la mascota necesita comer
    if (idx == 1)
        return " <-- Advertencia: Muy Cansado!"; // aca es por si la mascota esta cansada, necesita dormir
    if (idx == 2)
        return " <-- Advertencia: Muy Sucio!"; // aca la mascota necesita un baño
    if (idx == 3)
...
void truncarEstadisticas(Mascota &m)
{
    for (int i = 0; i < TOTAL_ESTADISTICAS; i++)
    {
        if (m.estadisticas[i] > LIMITE_MAXIMO)
            m.estadisticas[i] = LIMITE_MAXIMO; // compara las estadisticas para que no se pase del maximo
        else if (m.estadisticas[i] < LIMITE_MINIMO)
            m.estadisticas[i] = LIMITE_MINIMO; // compara las estadisticas para que no se pasen del minimo
    }
}
...
string nombreAccionRecomendada(int idx)
{
    ...
    if (idx == 0)
        return "Dar comida (opcion 1)"; // Alimentacion -> Dar comida
    if (idx == 1)
        return "Dormir     (opcion 3)"; // Energia      -> Dormir
    if (idx == 2)
        return "Bañar      (opcion 2)"; // Higiene      -> Bañar
    if (idx == 3)
        return "Jugar      (opcion 4)"; // Felicidad    -> Jugar
    return "una accion";
}
...
bool cargarSlot(int slot, Mascota &m)
{
    ...
    if (!getline(archivo, m.nombre) || m.nombre.empty())
    {
        archivo.close();
        return false;
    }
    if (!(archivo >> m.indiceEspecie))
    {
        archivo.close();
        return false;
    }
